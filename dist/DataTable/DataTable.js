...
var _PaginationSelect = _interopRequireDefault(require("./PaginationSelect/PaginationSelect"));
...
var _TableHeading = _interopRequireDefault(require("./TableHeading/TableHeading"));
...
function DataTable(_ref) {
  ...
  var batchDataWithPaginationSelect = function batchDataWithPaginationSelect(arr, size) {
    size = parseInt(size);
    var myArray = [];
  ...
  var _useState11 = (0, _react.useState)(batchDataWithPaginationSelect(rawDataWithSearch, numberOfRows)),
  ...
  }, [rawDataWithSearch]); // data of the current page
  ...
  }, [currentPage]); // on batchedData change, set the max Page again, and set the current page data
  ...
  }, [batchedData]); //trigger batch when number of rows select changes

  (0, _react.useEffect)(function () {
    setbatchedData(batchDataWithPaginationSelect(rawDataWithSearch, numberOfRows));
  }, [numberOfRows]); //sort data ascending
  ...
  (0, _react.useEffect)(function () {
    if (activeSorting) {
      if (sortingDirection === "asc") {
        setbatchedData(batchDataWithPaginationSelect(sortAsc(rawDataWithSearch), numberOfRows));
      } else {
        setbatchedData(batchDataWithPaginationSelect(sortDesc(rawDataWithSearch), numberOfRows));
      }
